...
class CorpusConfig:
    ...
    ``None`` means "use plain ``OPENAI_API_KEY``" against api.openai.com.
    Azure OpenAI users should pass ``https://<resource>.../openai/v1``.
    Local vLLM users should pass ``http://host:port/v1``.
    """

    embed_api_key: SecretStr | None
    embed_model: str
    embed_query_instruction: str | None = None
    """Optional instruction prepended to query text (Qwen-style embedders only)."""
